...
var model = {
  ...
  columns: {
	 "QUICK": {
      ...
      selectedIndex: 0,
      icon: 'style',
      items: [{title:RC('Quick1A'), desc:RC('Quick1C'), version:RC('Quick1D'), icon:String.fromCharCode(RC('Quick1E')), expage:"payloads/" + RC('Quick1B')}, {title:RC('Quick2A'), desc:RC('Quick2C'), version:RC('Quick2D'), icon:String.fromCharCode(RC('Quick2E')), expage:"payloads/" + RC('Quick2B')}, {title:RC('Quick3A'), desc:RC('Quick3C'), version:RC('Quick3D'), icon:String.fromCharCode(RC('Quick3E')), expage:"payloads/" + RC('Quick3B')}, {title:RC('Quick4A'), desc:RC('Quick4C'), version:RC('Quick4D'), icon:String.fromCharCode(RC('Quick4E')), expage:"payloads/" + RC('Quick4B')}]
    },
    "HEN": {
      index: 1,
      title: "HEN",
  ...
  "GTAV MENUS": {
	...
	"STYLE": {
      ...
      selectedIndex: 0,
      icon: "",
      items: [{title:"X-PROJECT MENU COLOUR PICKER", desc:"Change the colour of X-Project's menu items and icons", version:"1.0 by DEFAULTDNB & Leeful", icon:"", expage:"payloads/ColPicker.html"}, {title:"X-PROJECT MENU AUTO STYLER", desc:"Click to randomly change the colour of X-Project's menu items and icons", version:"1.0 by DEFAULTDNB & Leeful", icon:"", expage:"payloads/ColRandom.html"}, {title:"X-PROJECT MENU DEFAULT COLOUR", desc:"Changes the colour of X-Project's menu items back to the default colour", version:"1.0 by DEFAULTDNB & Leeful", icon:"", expage:"payloads/ColDefault.html"}]
	},
	"ABOUT": {
      index: 14,
      title: "ABOUT",
      selectedIndex: 0,
      icon: "help",
      items: [{title:"SETTINGS", desc:"Customize various X-Project Settings", version:"2.0 by Leeful", icon:"", expage:"payloads/settingsloader.html"}, {title:"CREDITS", desc:"Without these people none of this would be possible", version:"1.5 by DEFAULTDNB", icon:"fingerprint", expage:"payloads/creditsloader.html"}, {title:"DETAILS", desc:"Tell us your suggestions & issues", version:"1.0 by DEFAULTDNB", icon:"", expage:"payloads/detailsloader.html"},{title:"PAYLOAD STATISTICS", desc:"Logs how many times a payload has been loaded. (VTX HEN/DUMP/FTP only)", version:"1.0 POC by DEFAULTDNB", icon:"", expage:"payloads/statsloader.html"},{title:"CHANGELOG", desc:"Changes since previous version", version:"1.0 by DEFAULTDNB", icon:"info", expage:"payloads/changelogloader.html"}]
    },
	"★DEBUG": {
      index: 15,
      title: "★DEBUG",
...
onkeydown = function(f){
	 ...
	 if (RC('xmbcolor')==null) {SetMissingValues();}
     if(map[117] && map[37]) {window.open("payloads/"+LS.Combo1B, "content");} // R1 + D-Pad Left
else if(map[117] && map[39]) {window.open("payloads/"+LS.Combo2B, "content");} // R1 + D-Pad Right
else if(map[117] && map[40]) {window.open("payloads/"+LS.Combo3B, "content");} // R1 + D-Pad Down
else if(map[117] && map[118]){window.open("payloads/"+LS.Combo4B, "content");} // R1 + L2
